...
function summarizeCounts(c: ResetCounts): string {
  const parts: string[] = [];
  parts.push(
    `${c.responses_deleted} response${c.responses_deleted === 1 ? "" : "s"}`,
  );
  parts.push(
    `${c.progress_deleted} progress row${c.progress_deleted === 1 ? "" : "s"}`,
  );
  parts.push(
    `${c.bookings_deleted} booking${c.bookings_deleted === 1 ? "" : "s"}`,
  );
  if (c.calendar_events_deleted > 0) {
    parts.push(
      `${c.calendar_events_deleted} calendar event${
        c.calendar_events_deleted === 1 ? "" : "s"
      }`,
    );
  }
...
export function ResetCandidateModal({
  ...
  // Escape-to-close, but only when idle so admins don't accidentally dismiss
  // mid-reset and lose the server result.
  useEffect(() => {
  ...
  const placeholder = token.length <= 4 ? token : `${token.slice(0, 4)}…`;
      ...
      <div className="bm-modal">
        ...
        <div className="bm-modal-body">
          <div className="bm-modal-warning">
            ...
            <p>
              The candidate returns to <strong>Chapter 1 · Step 1</strong>{" "}
              with a clean slate. Cannot be undone.
            </p>
          </div>
